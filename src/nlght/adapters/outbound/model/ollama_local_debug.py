    ...
    async def call(self, messages: list[dict[str, Any]], *, temperature: float | None = None) -> None:
        ...
        if self._stream:
            ...
            for _round in range(10):
                ...
                logger.info(
                    "ollama.call.request | round=%d model=%s tools_in_request=%s",
                    _round, self._model,
                ...
                # verbatim, before any of our processing.
                logger.info(
                    "ollama.call.response_tool_calls | round=%d names=%s",
                    _round, [(tc.get("function") or {}).get("name") for tc in tool_calls_native],
                )
                logger.debug("ollama.call.response_tool_calls_raw | round=%d raw=%s", _round, tool_calls_native)
                ...
                logger.info(
                    "ollama.call.tool_loop | round=%d calls=%d dispatch_names=%s",
    ...
    async def stream(
        ...
        *,
        ...
        for _round in range(10):
            ...
            logger.info(
                "ollama.stream.request | round=%d model=%s tools=%s",
                _round, self._model,
            ...
            logger.info(
                "ollama.stream.tool_loop | round=%d calls=%d names=%s",
            ...
            for tc, result_text in zip(tool_calls_raw, results, strict=False):
                logger.info("ollama.tool_call | name=%s chars=%d", tc["name"], len(result_text))
            ...
            if terminal_names and any(tc.get("name") in terminal_names for tc in tool_calls_raw):
                logger.info("ollama.tool_loop_terminal | round=%d", _round)
                break
